...
|----------|-------------|
| (aucun) | Export + copie + import complet |
...
- Les embeddings sont copiés tels quels (pas de ré-indexation en prod)
- Les clés API sont synchronisées depuis `platform_config`
- Les données existantes en prod sont **remplacées** (pas de merge)
